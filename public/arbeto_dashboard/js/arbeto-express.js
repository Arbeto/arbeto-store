...
function loadOrders() {
  ...
  // Filter orders shipped via Arbeto Express
  arexOrders = allOrders
    .filter(
      (order) =>
        order.shippingCompany === "Arbeto Express" && order.status === "shipped"
    )
    .map((order) => ({
      ...order,
  ...
  container.innerHTML = filteredOrders
    ...
    <div class="order-card ${order.delivered ? "delivered" : ""}" id="card-${
      ...
      <div class="notes-section">
        ...
        <div class="add-note-form">
          <input type="text" class="note-input" id="noteInput-${
            order.id
          }" placeholder="اكتب ملاحظة للسائق...">
      ...
      <div class="shipping-cost-section">
        <div class="cost-display">
          ...
          <div style="display: flex; align-items: center; gap: 10px;">
            <span class="cost-value" id="costValue-${order.id}">${
        order.additionalCost
      }</span>
            <span style="font-size: 16px; color: #596d52;">جنيه</span>
            <button class="btn-reset-cost" onclick="openResetModal(${
              order.id
            })" title="إعادة تعيين">
        ...
        <div class="add-cost-form">
          ...
          <input type="number" class="cost-input" id="costInput-${
            order.id
          }" placeholder="0" min="0">
          <button class="btn-add-cost" onclick="addShippingCost(${
            order.id
          })">+</button>
        </div>
      </div>
      ...
      <div class="rating-section">
        <div class="rating-title">تقييم الزبون في التوصيل</div>
...
function updateStatistics() {
  ...
  const totalShipping = arexOrders.reduce(
    (sum, order) => sum + order.additionalCost,
    0
  );
